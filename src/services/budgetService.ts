...
export class BudgetService {
    static validateBudgetData(budgetData: Partial<IBudget>): void {
        const errors = [];
        ...
        if (budgetData.totalCost !== undefined && budgetData.totalCost < 0) {
            errors.push({ field: "totalCost", message: "Total cost must be a non-negative number." });
        }

        if (budgetData.totalHours !== undefined && budgetData.totalHours < 0) {
            errors.push({ field: "totalHours", message: "Total hours must be a non-negative number." });
        }

        if (budgetData.commissionPercentage !== undefined && (budgetData.commissionPercentage < 0 || budgetData.commissionPercentage > 100)) {
    ...
    public static async addBudgetConcepts(budgetId: string, budgetConcepts: IBudgetConcept[]): Promise<IBudget | null> {
        ...
        for (const concept of budgetConcepts) {
            const { concept: conceptId, quantity, costPrice, percentageRisk, effortHours, effortRisk, totalCost } = concept;

            // Validate required fields
            if (!conceptId || !quantity || !costPrice) {
                throw new Error("Each budget concept must have a concept ID, quantity, and cost.");
            }
        ...
        budget.budgetConcepts = [...budget.budgetConcepts, ...savedBudgetConceptIds];
        await budget.save();
    ...
    public static async addBudgetConcept(budgetId: string, budgetConcept: IBudgetConcept): Promise<IBudget | null> {
        const budget = await BudgetModel.findById(budgetId);
        if (!budget) {
            throw new Error("Budget not found");
        }
    ...
    public static async deleteBudgetConcept(budgetId: string, budgetConceptId: string): Promise<IBudget | null> {
        const budget = await BudgetModel.findById(budgetId);
        if (!budget) {
            throw new Error("Budget not found");
        }
